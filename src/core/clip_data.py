...
class ClipData:
    ...
    def from_qmime(cls, qmime: QMimeData) -> Optional["ClipData"]:
        ...
        if qmime.hasImage():
            # Validate encoded PNG bytes before Qt decodes a lazy clipboard
            # transfer that may have been interrupted in the source application.
            png_data = (
                bytes(qmime.data("image/png"))
                if qmime.hasFormat("image/png")
                else b""
            )
            if png_data:
                qimg = ImageUtils.deserialize(png_data)
            else:
                img = qmime.imageData()
                qimg = QImage(img) if isinstance(img, QImage) else QPixmap(img).toImage()

            if qimg.isNull():
                logger.warning("Ignored clipboard image because it could not be decoded")
                return None

            obj = cls(MimeType.IMAGE, qimg)

            obj.data_bytes = png_data or ImageUtils.serialize(qimg, "PNG")
    ...
    def create_preview_widget(self, max_height: int = 150, widget_factory: Optional[Callable[["ClipData"], QWidget]] = None) -> QWidget:
        ...
        if self.mime_type == MimeType.HTML:
            lbl.setText(html_preview_text(self.data))
